...
try {
  ...
  // Some serverless runtimes can fail to load the SDK due to module format differences.
  // We fallback to in-memory storage to keep the API available.
  Redis = null;
}
...
const redis = hasRedisConfig && Redis
  ? new Redis({
      url: redisUrl,
      token: redisToken,
    })
